...
#'
#' @param  na.rm  Whether to remove NA/NaN values; if those are not removed, output will usually be NA
#' @export
...
mase <- function(actual, predicted, step_size = 1, na.rm = F) {

    naive_start <- step_size + 1
    n <- as.numeric(length(actual))
    naive_end <- n - step_size

    sum_errors <- sum(ae(actual, predicted, na.rm = na.rm))
    naive_errors <- sum(ae(actual[naive_start:n], actual[1:naive_end], na.rm = na.rm))
